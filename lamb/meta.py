...
class TypeEnv(object):
    ...
    def _repr_html_(self):
        s = "<table>"
        s += (
            "<tr><td>Term mappings:&nbsp;&nbsp; </td><td>%s</td></tr>"
            % utils.dict_latex_repr(self.var_mapping)
        )
        s += (
            "<tr><td>Type mappings:&nbsp;&nbsp; </td><td>%s</td></tr>"
            % utils.dict_latex_repr(self.type_mapping)
        )
        s += (
            "<tr><td>Type variables:&nbsp;&nbsp; </td><td>%s</td></tr>"
    ...
    def try_add_var_mapping(self, vname, typ):
        ts = get_type_system()
        if vname in self.var_mapping:
            principal = self.try_unify(
                self.var_mapping[vname], typ, update_mapping=True
            )
    ...
    def intersect_merge(self, tenv):
        for v in tenv.type_mapping:
            if (
                v in self.type_var_set
                or len(tenv.type_mapping[v].bound_type_vars() & self.type_var_set) > 0
            ):
                self.add_type_mapping(v, tenv.type_mapping[v], defer=True)
        self.update_type_vars()
        for v in tenv.var_mapping:
...
def merge_tes(te1, te2, symmetric=True):
    ...
    * Non-symmetric: if `te1` is a term, return `te2` at the principal type.
    ...
    if principal is None:
        raise TypeMismatch(
            te1, te2, "Failed to merge typed expressions (incompatible types)"
        )
    te1_new = te1.try_adjust_type(principal)
    te2_new = te2.try_adjust_type(principal)
    if te1_new is None or te2_new is None:
        raise TypeMismatch(
            te1, te2, "Failed to merge typed expressions (type adjustment failed)"
        )
    if te1_new.term():
        if symmetric and te2_new.term() and not (te1_new == te2_new):
            raise TypeMismatch(
                te1, te2, "Failed to merge typed expressions; result is not equal"
            )
        return te2_new
...
class TypedExpr(object):
    ...
    def try_adjust_type_caching(
        self, new_type, derivation_reason=None, assignment=None, let_step=None
    ):
        cached = self._type_cache_get(new_type)
        if cached is not False:
            return cached
        if let_step is not None:
            result = let_step.try_adjust_type(
                new_type, derivation_reason=derivation_reason, assignment=assignment
            )
            # TODO: freshen variables again here?
        else:
            result = self.try_adjust_type(
                new_type, derivation_reason=derivation_reason, assignment=assignment
            )
    ...
    def try_adjust_type(self, new_type, derivation_reason=None, assignment=None):
        ...
        if self.type == unify_target:
            ...
            if self.term():
                ...
                result = self.try_adjust_type_local(
                    unify_target, derivation_reason, assignment, env
                )
    ...
    def try_adjust_type_local(self, unified_type, derivation_reason, assignment, env):
        ...
        logger.error(
            "Unimplemented `try_adjust_type_local` in class '%s'" % type(self).__name__
        )
        return None
    ...
    def subst(self, i, s, assignment=None):
        ...
        if unified is None:
            raise TypeMismatch(
                s, old, "Substitution for element %s of '%s'" % (i, repr(self))
            )
        if unified != s.type:
    ...
    def try_parse_binding_struc(cls, s, assignment=None, locals=None, vprefix="ilnb"):
        ...
        try:
            return BindingOp.try_parse_binding_struc_r(
                s, assignment=assignment, locals=locals, vprefix=vprefix
            )
    ...
    @classmethod
    def try_parse_paren_struc_r(
        cls, struc, assignment=None, locals=None, vprefix="ilnb"
    ):
        """Recursively try to parse a semi-AST with parenthetical structures
        matched."""
        expr = cls.try_parse_binding_struc(
            struc, assignment=assignment, locals=locals, vprefix=vprefix
        )
        if expr is not None:
            return expr
        ...
        for sub in struc:
            if isinstance(sub, str):
                ...
                sub_expr = cls.try_parse_paren_struc_r(
                    sub, assignment=assignment, locals=locals, vprefix=vprefix
                )
    ...
    @classmethod
    def try_parse_term_sequence(
        cls, s, lower_bound=1, upper_bound=None, assignment=None
    ):
        s = s.strip()
        if len(s) == 0:
            sequence = list()
        ...
        if i < len(s):
            ...
            while i < len(s):
                i = parsing.consume_char(
                    s, i, ",", "expected comma in variable sequence"
                )
                i = parsing.consume_whitespace(s, i)
                v, typ, i = cls.parse_term(
                    s, i=i, return_obj=False, assignment=assignment
                )
                if v is None:
    ...
    def try_parse_typed_term(cls, s, assignment=None, strict=False):
        ...
        seq = cls.try_parse_term_sequence(
            s, lower_bound=1, upper_bound=1, assignment=assignment
        )
        return seq[0]
    ...
    def expand_terms(cls, s, i=0, assignment=None, ignore=None):
        ...
        for t in terms:
            if t.start() + offset < i:
                ...
                continue
            (name, typ, end) = cls.parse_term(
                s, t.start() + offset, return_obj=False, assignment=assignment
            )
            if name is None:
            ...
            if typ is None:
                replace = (
                    'TypedExpr.term_factory("%s", typ=None, assignment=assignment)'
                    % (name)
                )
            else:
                replace = (
                    'TypedExpr.term_factory("%s", typ="%s", assignment=assignment)'
                    % (name, repr(typ))
                )
    ...
    def parse_term(cls, s, i=0, return_obj=True, assignment=None):
        ...
        ts = get_type_system()
        term_name, next = parsing.consume_pattern(
            s, i, r"([a-zA-Z0-9]+)(_)?", return_match=True
        )
        if not term_name:
            if return_obj:
                return (None, i)
        ...
        if return_obj:
            term = cls.term_factory(
                term_name.group(1), typ=typ, assignment=assignment, preparsed=True
            )
            return (term, end)
        else:
    ...
    def factory(cls, *args, assignment=None):
        ...
        if len(args) == 0:
            ...
            if (not operator.type.functional()) and operator.type_guessed:
                ...
                coerced_op = operator.try_coerce_new_argument(
        ...
        for part in self:
            if isinstance(part, TypedExpr):
                env = merge_type_envs(
                    env,
                    part.type_env(
                        constants=constants, target=target, free_only=free_only
                    ),
                )
        return env
    ...
    def compact_type_vars(
        self, target=None, unsafe=None, used_vars_only=True, store_mapping=False
    ):
    ...
    def freshen_type_vars(
        self, target=None, unsafe=None, used_vars_only=False, store_mapping=False
    ):
        history_env = self.get_type_env()
        if len(history_env.type_var_set) == 0:
            return self
    ...
    def simplify_all(self):
        ...
        for i in range(len(result.args)):
            ...
            if new_arg_i is not result.args[i]:
                dirty = True
                result = derived(
                    result.subst(i, new_arg_i),
                    result,
                    desc=("Recursive simplification of argument %i" % i),
    ...
    def reduce_all(self):
        ...
        for i in range(len(result.args)):
            ...
            if new_arg_i is not result.args[i]:
                ...
                result = derived(
                    next_step, result, desc=reason, subexpression=new_arg_i
                )
        self_dirty = False
    ...
    def calculate_partiality(self, vars=None):
        ...
        if condition is true_term:
            intermediate = derived(
                Partial(new_self, condition), self, "Partiality simplification"
            )
    ...
    def latex_str(self, **kwargs):
        ...
        # operators
            ...
            return ensuremath(
                text_op_in_latex(self.op) + self.args[0].latex_str(**kwargs)
            )
        else:  # Infix operator
            return ensuremath(
                "(%s)"
...
class ApplicationExpr(TypedExpr):
    def __init__(self, fun, arg, defer=False, assignment=None, type_check=True):
        if type_check and not defer:
            ...
            if tc_result is None:
                raise TypeMismatch(
                    fun, arg, "Function argument combination (unification failed)"
                )
    ...
    def try_adjust_type_local(self, new_type, derivation_reason, assignment, env):
        ...
        if new_fun_type is None:
            return None
        new_fun = fun.try_adjust_type(
            new_fun_type, derivation_reason=derivation_reason, assignment=assignment
        )
        if new_fun is None:
            return None
        new_arg = arg.try_adjust_type(
            new_arg_type, derivation_reason=derivation_reason, assignment=assignment
        )
        if new_arg is None:
            return None
    ...
    def fa_type_inference(cls, fun, arg, assignment):
        ...
        if fun.type != f_type:
            fun = fun.try_adjust_type_caching(
                f_type,
        ...
        if a_type != arg.type:
            arg = arg.try_adjust_type_caching(
                a_type,
                derivation_reason="Type inference (external)",
                assignment=assignment,
            )
...
class Tuple(TypedExpr):
    ...
    def random(cls, ctrl, max_type_depth=1, max_tuple_len=5, allow_empty=True):
        ...
        signature = [
            get_type_system().random_type(max_type_depth, 0.5) for i in range(length)
        ]
...
class TypedTerm(TypedExpr):
    ...
    def should_show_type(self, assignment=None):
        ...
        if suppress_constant_predicate_type:
            if (
                self.constant()
                and self.type.functional()
                and not isinstance(self.type, types.VariableType)
            ):
                if (
                    self.type.left == types.type_e
    ...
    def try_coerce_new_argument(self, typ, remove_guessed=False, assignment=None):
        if not self.type_guessed:
            return None
        coerced_op = self.term_factory(
            self.op, typ=self.type.add_internal_argument(typ), preparsed=True
        )
    ...
    def random(
        ...
        if typ is None:
            ...
            used_typed = [
                x for x in list(usedset) if (x.type == typ and x.variable() == is_var)
            ]
...
class CustomTerm(TypedTerm):
    ...
    def __init__(
        ...
        TypedTerm.__init__(
            self, varname, typ=typ, assignment=assignment, type_check=type_check
        )
...
class Partial(TypedExpr):
    ...
    def calculate_partiality(self, vars=None):
        ...
        return derived(
            Partial(new_body, new_condition), self, "Partiality simplification"
        )
    ...
    def try_adjust_type_local(self, unified_type, derivation_reason, assignment, env):
        ...
        result = tuple_version.try_adjust_type(
            unified_type, derivation_reason, assignment, env
        )
    ...
    def latex_str(self, **kwargs):
        if self.condition and self.condition != true_term:
            return ensuremath(
                ...
                % (self.body.latex_str(**kwargs), self.condition.latex_str(**kwargs))
            )
        else:
    ...
    def from_Tuple(cls, t):
        if isinstance(t, TypedExpr) and (not isinstance(t, Tuple) or len(t) != 2):
            raise parsing.ParseError(
                "Partial requires a Tuple of length 2.  (Received `%s`.)" % repr(t)
            )
...
class Disjunctive(TypedExpr):
                ...
                if r is not None:
                    if r.type in t_adjust:
                        raise parsing.ParseError(
                            ...
                            "for disjuncts '%s')" % (repr(t), repr(d), repr(disjuncts))
                        )
                    else:
                        t_adjust |= {r.type}
    ...
    def latex_str(self, disj_type=False, **kwargs):
        if disj_type:
            ...
            return ensuremath(
                "{Disjunctive}(\\left[%s\\right])"
                % (("\\mid{}").join([a.latex_str(**kwargs) for a in self.args]))
            )
    ...
    def try_adjust_type_local(self, unified_type, derivation_reason, assignment, env):
        ...
        for a in self.args:
            ...
            l.append(
                a.try_adjust_type(
                    t, derivation_reason=derivation_reason, assignment=assignment
                )
            )
        assert len(l) > 0
        if len(l) == 1:
    ...
    def random(cls, ctrl, max_type_depth=1, max_disj_len=3):
        ...
        length = random.choice(r)
        signature = {
            get_type_system().random_type(
                max_type_depth, 0.5, allow_variables=False, allow_disjunction=False
            )
...
class UnaryOpExpr(TypedExpr):
    ...
    def __init__(
        self, typ, op, arg1, op_name_uni=None, op_name_latex=None, tcheck_args=True
    ):
        if tcheck_args:
    ...
    def latex_str(self, **kwargs):
        if self.operator_style:
            return ensuremath(
                "%s %s" % (self.op_name_latex, self.args[0].latex_str(**kwargs))
            )
        else:
            return ensuremath(
                "%s(%s)" % (self.op_name_latex, self.args[0].latex_str(**kwargs))
...
class BinaryOpExpr(TypedExpr):
    ...
    def __init__(
        ...
        if tcheck_args:
            args = [
                self.ensure_typed_expr(arg1, typ),
...
class BinaryArrowExpr(BinaryOpExpr):
    ...
    def simplify(self):
        if self.args[0] == false_term:
            ...
            return derived(
                UnaryNegExpr(self.args[0]), self, desc="material implication"
            )
        elif self.args[1] == true_term:
            return derived(true_term, self, desc="material implication")
        elif self.args[0] == self.args[1]:
...
class BinaryGenericEqExpr(BinaryOpExpr):
    ...
    def simplify(self):
        if self.args[0] == self.args[1]:
            ...
            if isinstance(self.args[0].op, Number) and isinstance(
                self.args[1].op, Number
            ):
    ...
    class BinOp(BinaryOpExpr):
        ...
        def simplify(self):
            ...
            if isinstance(self.args[0].op, Number) and isinstance(
                ...
                return derived(
                    te(simplify_fun(self.args[0].op, self.args[1].op)),
                    self,
                    desc=op_uni,
                )
            else:
...
class SetContains(BinaryOpExpr):
    ...
    def random(cls, ctrl, max_type_depth=1):
        ...
        return SetContains(
            ctrl(typ=content_type), ctrl(typ=types.SetType(content_type))
        )
...
class TupleIndex(BinaryOpExpr):
    def __init__(self, arg1, arg2, type_check=True):
        ...
        if isinstance(arg2.op, Number):  # TODO better way to determine whether
            ...
            logger.warning("Using non-constant tuple index; not well-supported.")
    ...
    def latex_str(self, **kwargs):
        return ensuremath(
            "(%s[%s])"
            % (self.args[0].latex_str(**kwargs), self.args[1].latex_str(**kwargs))
        )
...
unary_symbols_to_op_exprs = {"~": UnaryNegExpr, "-": UnaryNegativeExpr}
...
op_symbols = set(unary_symbols_to_op_exprs.keys()) | set(
    binary_symbols_to_op_exprs.keys()
)
...
class BindingOp(TypedExpr):
        ...
        if op_class.allow_multivars or op_class.allow_novars:
            ...
            if len(var_list) != 1:
                raise parsing.ParseError(
                    ...
                    % (op_class.canonical_name, len(var_list)),
                    None,
                )
            return op_class(
                var_or_vtype=var_list[0][1],
    ...
    def __init__(
        ...
        if isinstance(var_or_vtype, TypedTerm):
            if varname is not None:
                logger.warning(
                    "Overriding varname '%s' with '%s'" % (varname, var_or_vtype.op)
                )
        ...
        if type_check:
            ...
            self.init_var_by_instance(
                self.var_instance.under_type_assignment(
                    body_env.type_mapping, merge_intersect=False
                )
            )
        else:
            self.init_body(body)
    ...
    def project_partiality_strict(b, body, condition):
        ...
        if isinstance(b, ConditionSet) or isinstance(b, LFun):
            ...
            return Partial(
                b_cls(b.var_instance, body), ForallUnary(b.var_instance, body)
            )
    ...
    def project_partiality_weak(b, body, condition):
        ...
        if isinstance(b, ForallUnary):
            return Partial(
                b_cls(b.var_instance, body), b_cls(b.var_instance, condition)
            )
        elif isinstance(b, ExistsUnary) or isinstance(b, ExistsExact):
            return Partial(
                b_cls(b.var_instance, body & condition),
                b_cls(b.var_instance, condition),
            )
        elif isinstance(
            b, IotaUnary
        ):  # does this lead to scope issues for the condition?
            return Partial(
                b_cls(b.var_instance, body & condition),
    ...
    def calculate_partiality(self, vars=None):
        ...
        if isinstance(new_body, Partial):
            if new_body.condition is true_term:
                return derived(
                    self.copy_local(self.var_instance, new_body),
                    self,
            ...
            if self.varname in new_body.condition.free_variables():
                if BindingOp.partiality_weak:
                    return derived(
                        self.project_partiality_weak(new_body.body, new_body.condition),
                        self,
                        "Partiality simplification",
                    )
                else:
                    return derived(
                        self.project_partiality_strict(
                            new_body.body, new_body.condition
                        ),
                        self,
                        "Partiality simplification",
                    )
                ...
                return derived(
                    Partial(new_self, new_condition), self, "Partiality simplification"
                )
        else:
            return derived(
                self.copy_local(self.var_instance, new_body),
    ...
    def try_parse_header(cls, s, assignment=None, locals=None):
        ...
        if op_name not in BindingOp.binding_operators:
            raise Error(
                "Can't find binding operator '%s'; should be impossible" % op_name
            )
        ...
        if len(split) != 2:
            ...
            raise parsing.ParseError(
                "Missing ':' in binding operator expression",
                s,
                None,
                met_preconditions=False,
            )
        ...
        var_seq = cls.try_parse_term_sequence(
            vname, lower_bound=None, upper_bound=None, assignment=assignment
        )
        return (op_class, var_seq, remainder)

    @classmethod
    def try_parse_binding_struc_r(
        cls, struc, assignment=None, locals=None, vprefix="ilnb"
    ):
        ...
        body = None
        try:
            body = TypedExpr.try_parse_paren_struc_r(
                new_struc, assignment=assignment, locals=locals, vprefix=vprefix
            )
            ...
            if isinstance(e, parsing.ParseError):
                ...
                raise parsing.ParseError(
                    ...
                    parsing.flatten_paren_struc(struc),
                    None,
                    e=e,
                )
        if body is None:
        ...
        result = BindingOp.binding_op_factory(
            op_class, var_list, body, assignment=assignment
        )
        return result
    ...
    def random(cls, ctrl, body_type=type_t, max_type_depth=1):
        ...
        variable = random_term(
            var_type, usedset=random_used_vars, prob_used=0.2, prob_var=1.0
        )
...
class ConditionSet(BindingOp):
    ...
    def __init__(
        self, var_or_vtype, body, varname=None, assignment=None, type_check=True
    ):
        body = self.ensure_typed_expr(body, assignment=assignment)
        super().__init__(
            var_or_vtype=var_or_vtype,
            typ=None,
            body=body,
            varname=varname,
    ...
    def latex_str(self, parens=True, **kwargs):
        return ensuremath(
            "\\{%s_{%s}\\:|\\: " % (self.varname, self.vartype.latex_str())
...
class ListedSet(TypedExpr):
    ...
    def to_condition_set(self):
        ...
        conditions = [
            BinaryGenericEqExpr(TypedTerm(varname, a.type), a) for a in self.args
        ]
        return ConditionSet(
            self.type.content_type, BinaryOrExpr.join(*conditions), varname=varname
        )
    ...
    def reduce_all(self):
        ...
        if dirty:
            ...
            new_result = derived(
                new_result, result, desc="Construction of set from reduced set members"
            )
            result = new_result
        return result
    ...
    def try_adjust_type_local(self, unified_type, derivation_reason, assignment, env):
        ...
        content = [
            a.try_adjust_type(
                inner_type, derivation_reason=derivation_reason, assignment=assignment
            )
            for a in self.args
        ]
...
class LFun(BindingOp):
    ...
    def try_adjust_type_local(self, unified_type, derivation_reason, assignment, env):
        ...
        left_principal = env.try_add_var_mapping(self.varname, unified_type.left)
        ...
        if self.type.right != unified_type.right:
            new_body = new_body.try_adjust_type(
                unified_type.right,
                derivation_reason=derivation_reason,
                assignment=assignment,
            )
...
def geach_combinator(gtype, ftype):
    ...
    combinator = LFun(
        gtype,
        LFun(ftype, LFun(ftype.left, body, varname="x"), varname="f"),
        varname="g",
    )
    return combinator
...
def fun_compose(g, f):
    ...
    defined above."""
    if not (
        g.type.functional() and f.type.functional() and g.type.left == f.type.right
    ):
...
class Derivation(object):
    ...
    def equality_display(self, content, style=None):
        ...
        n = display.DisplayNode(
            content=content,
            parts=[step[0] for step in l],
    ...
    def build_display_tree(self, recurse=False, parent=None, reason=None, style=None):
        ...
        for (expr, subreason, subexpression) in l:
            ...
            if subexpression and subexpression.derivation and (recurse):
                parts.append(
                    subexpression.derivation.build_display_tree(
                        recurse=recurse, parent=expr, reason=subreason, style=style
                    )
                )
            else:
                parts.append(
        ...
        return display.DisplayNode(
            content=parent, explanation=reason, parts=parts, style=node_style
        )
...
def derived(
    result, origin, desc=None, latex_desc=None, subexpression=None, allow_trivial=False
):
    ...
    if result.derivation is None:
        ...
        d = result.derivation
    result.derivation = derivation_factory(
        result,
        desc=desc,
        latex_desc=latex_desc,
        origin=origin,
...
def add_derivation_step(
    ...
    if te.derivation is None:
        ...
        d = te.derivation
    te.derivation = derivation_factory(
        result,
        desc=desc,
        latex_desc=latex_desc,
        origin=origin,
...
def add_subexpression_step(te, subexpr, desc=None, latex_desc=None):
    ...
    add_derivation_step(
        te, end, start, desc=desc, latex_desc=latex_desc, subexpression=subexpr
    )
    return te
...
def random_term(
    ...
    return TypedTerm.random(
        random_ctrl_fun=None,
        typ=typ,
        blockset=blockset,
        usedset=usedset,
        prob_used=prob_used,
...
def random_lfun(typ, ctrl):
    ...
    body_type = typ.right
    variable = random_term(
        input_type, usedset=random_used_vars, prob_used=0.2, prob_var=1.0
    )
    random_used_vars |= {variable}
...
def random_lfun_force_bound(typ, ctrl):
    ...
    random_used_vars |= {variable}
    return LFun(
        variable, random_pred_combo_from_term(body_type, ctrl, usedset=random_used_vars)
    )
...
class MetaTest(unittest.TestCase):
    ...
    def test_parse(self):
        ...
        self.assertEqual(
            TypedExpr.factory("(P_<e,t>(x_e) & P_<e,t>(x_e)) | P_<e,t>(x_e)"), self.body
        )
        # parenthesis reduction
        self.assertEqual(
            TypedExpr.factory("((P_<e,t>(x_e) & P_<e,t>(x_e)) | (P_<e,t>(x_e)))"),
            self.body,
        )
    ...
    def test_repr_parse_0(self):
        test_repr_parse_abstract(self, 0)

    def test_repr_parse_1(self):
        test_repr_parse_abstract(self, 1)

    def test_repr_parse_2(self):
        test_repr_parse_abstract(self, 2)

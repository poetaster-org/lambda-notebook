...
def equality_table(lines):
    ...
    for l in lines:
        ...
        if i > 0:
            ...
            first_cell = ""
        subelement_with_text(row, "td", text=first_cell, style="padding-right:5px")
...
def alternative_explanation(expl_str, alternative_str):
    ...
    e.append(
        element_with_text(
            "span", " (or: " + alternative_str + ")", style="font-size:x-small;"
        )
    )
    return e
...
class DisplayNode(object):
    ...
    def render(self, **kwargs):
        return self.style.render(self.content, self.explanation, self.parts, **kwargs)

    def build_tree(self, **kwargs):
        # this may throw an exception if the style doesn't support build_tree
        return self.style.build_tree(
            self.content, self.explanation, self.parts, **kwargs
        )

    def _repr_html_(self):
        return ElementTree.tostring(self.render(), encoding="unicode", method="html")
...
class HTMLNodeDisplay(Styled):
    ...
    def padding_style(self, **kwargs):
        ...
        return "".join([("%s:%s;" % (k, padding[k])) for k in padding if padding[k]])
    ...
    def vertical_divs(self, lines, **kwargs):
        ...
        e = Element("div", style="display:table;table-layout:auto;", align=align)
        for l in lines:
            row = SubElement(
                e,
                "div",
...
class TDBoxDisplay(HTMLNodeDisplay):
    ...
    def render_parts(self, parts, **kwargs):
        ...
        e = Element("div", style="display: table;")
        join = Element(
            "div", style="align: center; vertical-align: middle; display: table-cell;"
        )
        join.append(element_with_text("span", text="*", style=("padding:1em;")))
    ...
    def render(self, content, explanation, parts, **kwargs):
        ...
        e = Element(
            ...
            align=align,
            style=(
                "display:table; margin:5px; border-collapse: collapse;"
                + self.border_style(**kwargs)
            ),
        )
        parts_row = SubElement(
            e,
            "div",
            align="center",
            style="display:table-row;border-bottom:1px solid #848482",
        )
        parts_inter = SubElement(
            parts_row, "div", style="display:table-cell;vertical-align:middle;"
        )
        parts_inter.append(self.render_parts(parts, **kwargs))
        if explanation is not None:
            expl_inter = SubElement(
                parts_row,
                "div",
        ...
        content_row = SubElement(
            e,
            "div",
            align="center",
            style=("display:table-row;" + self.padding_style(**kwargs)),
        )
        content_cell = SubElement(
            content_row, "div", align=align, style="display:table-cell;"
        )
...
class TDProofDisplay(HTMLNodeDisplay):
    ...
    def render(self, content, explanation, parts, **kwargs):
        align = self.get_style(kwargs, "align", "center")
        e = Element(
            "div", align=align, style=("display:table;" + self.border_style(**kwargs))
        )
        parts_row = SubElement(e, "div", align="center", style="display:table-row;")
        parts_inter = SubElement(
            parts_row,
            "div",
        ...
        if explanation is not None:
            ...
            expl_inter = SubElement(
                mid, "div", style="display:table-cell;vertical-align:middle;"
            )
            expl_inter2 = SubElement(
                expl_inter,
                "div",
        ...
        content_inter = SubElement(
            e,
            "div",
            align="center",
            style=("display:table-row;" + self.padding_style(**kwargs)),
        )
        content_inter.append(self.render_content(content, align="center", **kwargs))
...
class TreeNodeDisplay(HTMLNodeDisplay):
    def render_node(self, content, explanation, **kwargs):
        ...
        kwargs["padding"] = None
        expl_div = SubElement(e, "div", style="align-self:end;justify-self:right;")
        ...
        content_div = SubElement(e, "div", style="justify-self:left;")
    ...
    def tree_layout(self, content, explanation, parts, **kwargs):
        ...
        # object overall...
        return svgling.html.draw_tree(
            tree, horiz_spacing=svgling.core.HorizSpacing.TEXT
        )
...
class LRDerivationDisplay(HTMLNodeDisplay):
    ...
    def render_parts(self, parts, **kwargs):
        align = self.get_style(kwargs, "align", "center")
        e = Element("div", align=align, style="display:table;border-collapse:collapse;")
    ...
    def render(self, content, explanation, parts, parent_table=None, **kwargs):
        ...
        if explanation is not None or len(parts):
            ...
            if explanation is not None:
                expl_row = SubElement(sub_table, "div", style="display:table-row;")
                expl_row.append(self.render_explanation(explanation, **kwargs))
            if len(parts):
                parts_row = SubElement(
                    sub_table, "div", style="display:table-row;vertical-align:bottom;"
                )
...
def deriv_style(style):
    ...
    if deriv_style == DerivStyle.PROOF:
        ...
        defs = {"lr-padding": None, "padding": None}
...
def leaf_style(style):
    ...
    if deriv_style == DerivStyle.TREE:
        defs = {"definiendum": True, "lr-padding": "1em"}

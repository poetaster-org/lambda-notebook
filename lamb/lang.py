...
class CompositionFailure(Exception):
    ...
    def items_str(self, latex=False):
        return " * ".join([self.item_str(i, latex=latex) for i in self.items])
...
class Composable(object):
    ...
    def tree(self, derivations=False, recurse=True, style=None):
        return self.build_display_tree(
            derivations=derivations, recurse=recurse, style=style
        )
...
class Assignment(collections.MutableMapping):
    ...
    def text(self):
        if isinstance(self.base, Assignment):
            a_strs = [("%s/%s" % (self.store[k][0], k)) for k in self.store.keys()]
    ...
    def latex_str(self):
        ...
        if isinstance(self.base, Assignment):
            a_strs = [
                ("%s/%s" % (self.store[k]._repr_latex_(), k)) for k in self.store.keys()
            ]
            return ensuremath("{%s}^{%s}" % (self.base.latex_str(), ",".join(a_strs)))
        else:
                ...
                if a[s.op].term() and s.op == a[s.op].op:
                    ...
                    if latex:
                        out_l.append(
                            ensuremath(
                                "%s_\{%s\} = %s"
                                % (s.op, a[s.op].type.latex_str(), a[s.op].latex_str())
                            )
                        )
                    else:
...
class SingletonComposable(Composable):
    ...
    def __repr__(self):
        ...
        return text_inbr(self.name) + (
            "%s = %s" % (self.assign_controller.render(latex=False), repr(self.content))
        )
    ...
    def short_str(self, latex=False, i=None, **args):
        ...
        if latex:
            return ensuremath(
                inbrs(self.name + istr, self.assign_controller.render(latex=True))
                + self.type_str_latex()
            )
        else:
    ...
    def latex_str(self, i=None):
        ...
        return (
            ensuremath(self.short_str(latex=True, i=i) + " \\:=\\: ")
...
class CompositionTree(Tree, Composable):
        ...
        if self.denotations is None or (override and not self.constant):
            if isinstance(self.label(), str):
                placeholder = TreeComposite(
                    content=PlaceholderTerm(
                        self.label().lower(), self, system=self.system
                    ),
                    mode="Placeholder",
                    source=self,
                )
    ...
    def compose(self, other=None, override=True, assignment=None):
        ...
        if self.denotations is None:
            ...
            if not self.failed():
                self.denotations = self.system.compose_container(
        ...
        if children:
            for c in self.children:
                if isinstance(c, str):
                    ...
                    c_list.append(
                        c.short_str(latex=latex, children=False, force_brackets=False)
                    )
    ...
    def percolate_failures(self):
        ...
        if len(child_failures):
            self.denotations = CompositionResult(
                self, list(), child_failures, source=self
            )
    ...
    def build_display_tree(self, derivations=False, recurse=True, style=None):
        ...
        for i in range(len(self)):
            try:
                part_i = self[i].build_display_tree(
                    recurse=recurse, derivations=derivations, style=style
                )
            except AttributeError:
                leaf_style = display.leaf_style(style)
                part_i = display.DisplayNode(content=self[i], style=leaf_style)
            parts.append(part_i)
        if self.composed():
            ...
            if leaf_style.get_style(None, "definiendum", True):
                node_parts.append(
                    self.short_str(latex=True, children=False, force_brackets=True)
                )
            node_parts.append(s)
        ...
        if isinstance(composable, meta.TypedExpr):
            ...
            if isinstance(composable, Item) or isinstance(composable, Items):
                if not system.has_item(composable):
                    meta.logger.info(
                        "Adding item to lexicon before composing: " + repr(composable)
                    )
            ...
            if isinstance(composable, CompositionTree):
                ...
                return CompositionTree(
                    name, children=None, denotation=composable, system=system
                )
...
class TreeComposite(Composite, Tree):
    ...
    def build_display_tree(self, derivations=False, recurse=True, style=None):
        defaults = {
            "direction": display.Direction.TD,
            "border": False,
            "expl_style": "bracket",
        }
        ...
        if self.content is not None:
            ...
            content_str = "N/A"
        if self.placeholder():
            node_content = display.DisplayNode(parts=[content_str], style=int_style)
            expl = None
        else:
            if self.mode:
                ...
                if len(collapsed) > 0:
                    expl = display.alternative_explanation(
                        expl, self.collapsed_compose_str()
                    )
            else:
                expl = None
            ...
            if expl is not None and len(parts) == 0:
                ...
                parts.append(
                    display.DisplayNode(
                        parts=[self.short_str_latex()], style=leaf_style
                    )
                )
                if derivations and self.content.derivation is not None:
                    node_content = self.content.derivation.equality_display(None)
                else:
                    node_content = display.DisplayNode(
                        parts=[content_str], style=int_style
                    )
            else:
                if derivations and self.content.derivation is not None:
                    ...
                    node_content = display.DisplayNode(
                        parts=node_parts, style=int_style
                    )
        ...
        return display.DisplayNode(
            content=node_content, explanation=expl, parts=parts, style=final_style
        )
...
class CompositionResult(Composable):
    ...
    def build_summary_for_tree(self, style=None):
        ...
        if self.failed():
            ...
            if len(failed_modes):
                failed_modes = [
                    display.alert_span("failed: " + ", ".join(failed_modes))
                ]
            else:
                failed_modes = []
            return display.DisplayNode(
                parts=["Composition Failure!"] + failed_modes, style=leaf_style
            )
        else:
            n = 0
            parts = list()
            for composite in self.results:
                span = display.element_with_text(
                    "span",
        ...
        for f in self.failures:
            if isinstance(f, CompositionResult):
                if f.source is None:
                    ...
                    s += "Inheriting composition failure from %s. " % f.name
                if latex:
                    s += f.latex_str()
                else:
                    s += str(f)
            elif isinstance(f, Composite):
                if not preconditions and isinstance(f.content, PreconditionFailure):
    ...
    def full_trace_latex(self, subpaths=False):
        ...
        if len(self.results) == 0:
            s += (
                "No successful paths -- tracing failures...<br />\n"
                ...
                for step in steps:
                    if step is path:
                        s += (
                            latex_indent()
                            + ("Step %i: " % step_i)
                            + step.compose_str(latex=True, collapsed=(not subpaths))
                            + "<br />\n"
                        )
                    else:
                        s += (
    ...
    def prune(self, i, reason=None):
        ...
        result = Composite(
            self.results[i], CompositionFailure(self.results[i], reason=reason)
        )
        ...
        if len(self.results) == 0:
            ...
            for composite in self.results:
                ...
                if num == 1:
                    ...
                    s += (
                        '%s &nbsp;&nbsp;<span style="font-size:small">(%i equivalent items)</span>'
...
class Item(TreeComposite):
    ...
    def build_display_tree(self, derivations=False, recurse=None, style=None):
        ...
        if not self.content:
            return display.DisplayNode(
                parts=[self.short_str_latex(), "N/A"], style=leaf_style
            )
        else:
            return super().build_display_tree(
                derivations=derivations, recurse=recurse, style=style
            )
    ...
    def set_descriptions_from_source(self, source):
        ...
        if isinstance(source, meta.TypedExpr):
            ...
            if isinstance(source, types.FunctionType):
                self.desc = "python function '%s.%s'" % (
                    source.__module__,
                    source.__name__,
                )
                self.latex_desc = self.desc
            else:
                self.desc = "python object '%s' of %s" % (
                    source.__name__,
                    source.__class__,
                )
...
class BinaryCompositionOp(CompositionOp):
    ...
    def __call__(self, i1, i2, assignment=None):
        ...
        if (not self.allow_none) and (i1.content is None or i2.content is None):
            raise CompositionFailure(i1, i2, reason="%s disallows None" % self.name)
    ...
    def __call__(self, tree, assignment=None):
        if not self.preconditions(tree):
            ...
            raise PreconditionFailure(
                tree, reason="Failed preconditions for %s" % self.name
            )
        if not self.allow_none:
            for d in tree:
                if d.content is None:
                    raise CompositionFailure(
                        *tree.children, reason="%s disallows None" % self.name
                    )
...
class LexiconOp(TreeCompositionOp):
    ...
    def freshen(self, tree, c):
        ...
        fresh = [TreeComposite(content=r, source=tree, mode=self) for r in contents]
    ...
    def lookup(self, t, assignment=None):
        ...
        if isinstance(t, str):
            ...
            isinstance(t, TreeComposite) and t.label() is None and t.source is not None
        ):
            name = t.source.label()
        elif (
            t.label() is not None
        ...
        if den is None:
            raise CompositionFailure(
                t, reason="No lexical entry for '%s' found." % name
            )
...
def unary_factory(meta_fun, name, typeshift=False, reduce=True):
    ...
    return UnaryCompositionOp(
        name, op_fun, typeshift=typeshift, reduce=reduce, source=meta_fun
    )


def binary_factory_uncurried(meta_fun, name, reduce=True, combinator_source=None):
    """Factory function to construct a binary composition operation given some
    function."""

    def op_fun(arg1, arg2, assignment=None):
        result = meta_fun(
            arg1.content.under_assignment(assignment),
...
def binary_factory(meta_fun, name, reduce=True, commutative=False):
    ...
    r = BinaryCompositionOp(
        name, op_fun, commutative=commutative, reduce=reduce, source=meta_fun
    )
    return r
...
class PlaceholderTerm(meta.TypedTerm):
    ...
    def __init__(
        self, varname, placeholder_for, system=None, assignment=None, type_check=True
    ):
        meta.TypedTerm.__init__(
            self,
            varname,
            types.UnknownType(),
    ...
    def latex_str(self, **kwargs):
        return ensuremath(
            inbrs(self.placeholder_name(), self.assign_controller.render(latex=True))
            + "_{"
            + self.type.latex_str()
            + "}"
        )
    ...
    def __repr__(self):
        # TODO: make this parse?
        return text_inbr(self.placeholder_name()) + self.assign_controller.render(
            latex=False
        )
    ...
    def copy_local(self, type_check=True):
        result = PlaceholderTerm(
            self.op, self.placeholder_for, system=self.system, type_check=type_check
        )
        result.let = self.let
...
class CompositionSystem(object):
    ...
    def add_binary_rule(self, combinator, name, reduce=True, commutative=False):
        rule = binary_factory(combinator, name, reduce=reduce, commutative=commutative)
        self.add_rule(rule)
        return rule

    def add_binary_rule_uncurried(self, fun, name, reduce=True, combinator_source=None):
        rule = binary_factory_uncurried(
            fun, name, reduce=reduce, combinator_source=combinator_source
        )
        self.add_rule(rule)
        return rule
    ...
    def compose(self, *items, assignment=None, block_typeshift=False):
        ...
        container object."""
        return self.compose_iterables(
            *items, assignment=assignment, block_typeshift=block_typeshift
        )
        ...
        for mode in self.rules:
            ...
            for order in orders:
                try:
                    ...
                    ret.extend(result)
                except (CompositionFailure, TypeMismatch, parsing.ParseError) as e:
                    ...
                    if arity == 1:
                        ...
                        ret.failures.append(
                            BinaryComposite(order[0], order[1], e, mode=mode)
                        )
    ...
    def last_resort_shift(self, *items, assignment=None):
        ...
        for i in range(1, self.typeshift_depth):
            ...
            for n in range(len(new_items)):
                new_i_n = self.do_typeshifts(
                    new_items[n], depth=i, assignment=assignment
                )
                new_items[n] = new_i_n
            ...
            if typeshifts_changed:
                result = self.compose(
                    *new_items, assignment=assignment, block_typeshift=True
                )
    ...
    def compose_iterables(self, *l, assignment=None, block_typeshift=False):
        ...
        if len(failures):
            return CompositionResult(
                l, list(), failures, source=self.source_from_list(*l)
            )

        iters = [x.content_iter() for x in l]
        result = self.compose_iterators(
            *iters, assignment=assignment, block_typeshift=block_typeshift
        )
    ...
    def compose_iterators(self, *iters, assignment=None, block_typeshift=False):
        ...
        for seq in itertools.product(*iters):
            r.extend(
                self.compose_raw(
                    *seq, assignment=assignment, block_typeshift=block_typeshift
                )
            )
        return r
    ...
    def compose_container(self, c, assignment=None, block_typeshift=False):
        ...
        r = self.compose_iterators(
            c.flatten_iter(), assignment=assignment, block_typeshift=block_typeshift
        )
...
class TreeCompositionSystem(CompositionSystem):
    ...
    def td_df_lr_gen(self, tree, parent, path_from_par, len_fun, full_path=None):
        ...
        if len_fun(tree) == 0:
            ...
            for i in range(len_fun(tree)):
                yield from self.td_df_lr_gen(
                    tree[i], tree, i, len_fun, full_path=full_path + (i,)
                )
    ...
    def compose(self, *composables, override=True, assignment=None, unary_extend=False):
    ...
    def unary_extend(self, c, override=True, assignment=None):
        return self.compose(
            c, override=override, assignment=assignment, unary_extend=True
        )
...
def tree_nn_fun(t, assignment=None):
    return UnaryComposite(
        t[0], content=t[0].content.under_assignment(assignment), source=t
    )
...
def tree_pm_fun(t, assignment=None):
    ...
    ts = meta.get_type_system()
    if not (
        ts.eq_check(t[0].type, type_property) and ts.eq_check(t[1].type, type_property)
    ):
...
def tree_pa_metalanguage_fun(t, assignment=None):
    ...
    f = meta.LFun(
        types.type_e, t[1].content.under_assignment(new_a), varname=outer_vname
    )
...
def tree_pa_sbc_fun(t, assignment=None):
    ...
    vname = "var%i" % index
    f = meta.LFun(
        types.type_e, t[1].content.under_assignment(assignment), varname=vname
    )
    return BinaryComposite(t[0], t[1], f, source=t)
...
class IndexedPronoun(Item):
    ...
    def short_str(self, latex=False, i=None, **kwargs):
        ...
        if latex:
            return ensuremath(
                latex_super(
                    inbr_raw(self.namefun(latex=True) + istr),
...
def pa_fun(binder, content, assignment=None):
    ...
    outer_vname = "x"
    inner_fun = meta.LFun(
        types.type_e, content.content.under_assignment(assignment), vname
    )
    # totally brute force...
    ...
    return BinaryComposite(
        ...
        (
            fun.content.under_assignment(assignment)(
                arg.content.under_assignment(assignment)
            )
        ).reduce(),
    )
...
def pm_fun(fun1, fun2, assignment=None):
    ...
    ts = meta.get_type_system()
    if not (
        ts.eq_check(fun1.type, type_property) and ts.eq_check(fun2.type, type_property)
    ):
...
def sbc_pa(binder, content, assignment=None):
    ...
    bound_var = meta.term(vname, types.type_e)
    f = meta.LFun(
        types.type_e, content.content.calculate_partiality({bound_var}), vname
    )
    return BinaryComposite(binder, content, f)
...
def setup_type_driven():
    ...
    td_system = CompositionSystem(
        rules=[fa, pm, pa], basictypes={type_e, type_t}, name="H&K simple"
    )
...
def setup_hk_chap3():
    ...
    hk3_system = TreeCompositionSystem(
        rules=[tfa_l, tfa_r, tpm, tpa, nn],

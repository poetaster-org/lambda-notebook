...
def try_parse_item_name(s, env=None, ambiguity=False):
    ...
    if lex_name != match.group(1):
        ...
        meta.logger.info(
            "Exporting item ||%s|| to python variable `%s`."
...
def parse_line(s, env=None, transforms=None, ambiguity=False):
    ...
    try:
        s = remove_comments(s)
        if len(s.strip()) > 0:
            (accum, env) = parse_equality_line(
                s, transforms=transforms, env=env, ambiguity=ambiguity
            )
            return (accum, env)
        else:
            return (dict(), env)
...
def latex_output(accum, env):
    ...
    for k in accum.keys():
        if isinstance(accum[k], meta.TypedExpr):
            ...
            print(
                "(Unknown class '%s') %s \\:=\\: %s" % (accum[k].__class__, k, accum[k])
            )
    return MiniLatex("<br />\n".join(lines))
...
def parse_paren_str(s, i, type_sys=None):
    ...
    if len(stack) != 0:
        raise ParseError(
            "Unbalanced '%s...%s' expression at end of string"
            % (stack[-1], brackets[stack[-1]]),
            s,
            i,
        )
    return (seq, i)
...
def parse_paren_str_r(s, i, stack, initial_accum=None, type_sys=None):
    ...
    while i < len(s):
        if s[i] == "_" and type_sys != None:
            ...
            r, new_i = parse_paren_str_r(
                s, i, stack, initial_accum=stack[-1], type_sys=type_sys
            )
            if len(accum) > 0:
                seq.append(accum)
                accum = ""
            ...
            if len(stack) > 0 and s[i] == brackets[stack[-1]]:
                ...
                raise ParseError(
                    "Unbalanced '%s...%s' expression" % (close_brackets[s[i]], s[i]),
                    s,
                    i,
                )
        else:
...
def macro_parse_r(struc, parse_fun, h, vnum=1, vprefix="ilnb", always_var=True):
    ...
    for sub in struc:
        if isinstance(sub, str):
            ...
            (sub_str, new_hash, vnum) = macro_parse_r(
                sub, parse_fun, h, vnum, vprefix=vprefix, always_var=always_var
            )
            h = new_hash
